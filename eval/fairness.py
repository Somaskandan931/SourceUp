...
def load_data ( test_frac=0.2, seed=42 ) -> Tuple[pd.DataFrame, pd.DataFrame] :
    ...
    # ================================================================
    # FIX: Handle location column properly
    # ================================================================

    # Check if location column exists (normalized name)
    has_location = "location" in df.columns

    if not has_location and os.path.exists( CLEAN_DATA ) :
        # Try to load location from clean data
        try :
            ...
            if "location" in df_clean.columns :
                ...
                train_len = len( df )
                if clean_len >= train_len :
                    df["location"] = df_clean["location"].values[:train_len]
                else :
                    # Pad with synthetic data if clean data is shorter
                    df["location"] = list( df_clean["location"].values ) + ["Unknown"] * (train_len - clean_len)
                print( "   ✅ Location column merged from suppliers_clean.csv" )
                has_location = True
            else :
                print( "   ⚠️  No 'location' column in clean data" )
    ...
    if not has_location :
        ...
        if "location_match" in df.columns :
            ...
            # Ultimate fallback: random assignment with some logic
            print( "   ⚠️  location_match also missing — using random assignment" )
            rng_loc = np.random.default_rng( 42 )
            cities = ["Mumbai", "Delhi", "Chennai", "Bangalore", "Kolkata", "Pune"]
            df["location"] = rng_loc.choice( cities, size=len( df ) )

    # ================================================================
    # Handle null/empty location values
    # ================================================================
    df["location"] = df["location"].fillna( "Unknown" )
    df["location"] = df["location"].astype( str )

    # Clean up common patterns
    df["location"] = df["location"].str.replace( r'\[.*?\]', '', regex=True )  # Remove list brackets
    df["location"] = df["location"].str.strip()

    # Set default for empty strings
    df.loc[df["location"] == "", "location"] = "Unknown"
    df.loc[df["location"] == "nan", "location"] = "Unknown"
    ...
    print( f"      {df['city_tier'].value_counts().to_string()}" )

    # ================================================================
    # Split data by query (no overlap)
    # ================================================================
...
def predict(model, df: pd.DataFrame) -> np.ndarray:
    ...
    if hasattr(model, 'predict'):
        ...
        # If model is a tuple, use the first element
        return model[0].predict(X)
    else:
        # Fallback: return zeros
        print("  ⚠️  Model prediction failed, returning zeros")
...
def plot_score_distributions(df_test_pred: pd.DataFrame):
    ...
    if "city_tier" not in df_test_pred.columns:
        print("  ⚠️  No city_tier column found, skipping score distribution plot")
        return

    # Find the score column (try different possible names)
    score_col = None
    for col in ["pred_score", "score", "predicted_score", "model_score"]:
        if col in df_test_pred.columns:
            score_col = col
            break

    if score_col is None:
        print("  ⚠️  No score column found, skipping score distribution plot")
        return
    ...
    if len(df_plot) == 0:
        print("  ⚠️  No data available for score distribution plot")
        return

    try:
        sns.violinplot(
            data=df_plot, x="city_tier", y=score_col,
        ...
        print(f"  ⚠️  Could not create violin plot: {e}")
...
def run_fairness_analysis():
    ...
    df_train, df_test = load_data(test_frac=0.2)  # Note: returns both!
